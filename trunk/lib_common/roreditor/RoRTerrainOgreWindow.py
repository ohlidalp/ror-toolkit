...
class RoRTerrainOgreWindow(wxOgreWindow):
	...
	def SceneInitialisation(self):
		...
		mat = ogre.MaterialManager.getSingleton().create("matline_ideal","debugger")
		mat.setReceiveShadows(False)
		mat.getTechnique(0).setLightingEnabled(True)
		...
		mat = ogre.MaterialManager.getSingleton().create("matline_real","debugger")
		mat.setReceiveShadows(False)
		mat.getTechnique(0).setLightingEnabled(True)
		...
		self.linenode = []
		for i in range(0, 10):
			self.line.append(self.sceneManager.createManualObject("lineobj"+str(i)))
...
"""		
	...
	def getTerrainHeight(self, pos):
		ray = ogre.Ray(pos, ogre.Vector3.NEGATIVE_UNIT_Y)
	...
	def onMouseEvent(self, event):
		...
		if event.LeftDown():
			...
			for i in range(0, self.spline.getNumPoints() - 1):
				point = self.spline.getPoint(i)
				self.line[0].position(point+ogre.Vector3(0,1,0))
			self.line[0].end()

			# 1.5 draw clicked line points
			self.line[4].clear()
			self.line[4].begin("matline", ogre.RenderOperation.OT_LINE_LIST)
			for i in range(0, self.spline.getNumPoints()):
				point = self.spline.getPoint(i)
				self.line[4].position(point)
				self.line[4].position(point+ogre.Vector3(0,2,0))
			self.line[4].end()
			...
			# draw red road line
			self.line[2].clear()
			self.line[2].begin("matline_real", ogre.RenderOperation.OT_LINE_STRIP)
			factor = 3
			for i in range(0, self.spline.getNumPoints() * factor):
				point = self.spline.interpolate(i/float(self.spline.getNumPoints() * factor))
				self.line[2].position(point+ogre.Vector3(0,1,0))
			self.line[2].end()

			# draw red road line - points
			self.line[3].clear()
			self.line[3].begin("matline_real", ogre.RenderOperation.OT_LINE_LIST)
			factor = 3
			for i in range(0, self.spline.getNumPoints() * factor):
				point = self.spline.interpolate(i/float(self.spline.getNumPoints() * factor))
	...
	def onKeyDown(self,event):
		...
		"""
			...
			for i in range(0, self.spline.getNumPoints() * factor):
				...
				rot = point.getRotationTo(nextpoint)
