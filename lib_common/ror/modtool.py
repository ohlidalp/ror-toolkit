...
class ModTool:
	...
	def work(self, mode, targetfile, verbose, dryrun, installtarget=None):
		...
		if mode in ['createmod']:
			...
			if dc.invalid:
				log().error("### target invalid! (Target not found)")
				log().info("### please use the 'listinstalled' mode to get valid createmod targets")
				return None
	...
	def getUniqueID(self):
		UNIQUEIDURL = "http://repository.rigsofrods.com/uniqueid/"
		try:
			log().info("trying to get a unique ID from the repository ...")
			f = urllib.urlopen(UNIQUEIDURL)
			lines = f.readlines()
			for line in lines:
				if line[0:9] == "<!--ID-->":
					id = line[9:].strip()
